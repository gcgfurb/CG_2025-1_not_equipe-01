...
namespace gcgcg
{
  public class Mundo : GameWindow
  {
    ...
    private Spline objetoSelecionado = null;
    private Transformacao4D matrizGrafo = new();

    private Circulo circuloMove;
    private Retangulo retanguloLimite;

    private Ponto pontoCentroCirculoMenor;
    ...
    protected override void OnUpdateFrame(FrameEventArgs e)
    {
      ...
      else if (estadoTeclado.IsKeyPressed(Keys.C))
      {
        objetoSelecionado.MoverCima();
      }
      else if (estadoTeclado.IsKeyPressed(Keys.B))
      {
        objetoSelecionado.MoverBaixo();
      }
      else if (estadoTeclado.IsKeyPressed(Keys.KeyPadAdd))
      {
        objetoSelecionado.SplineQtdPonto(1);
      }
    ...
    private void AtualizarCentroPonto()
    {
      pontoCentroCirculoMenor.PontosAlterar(new Ponto4D(circuloMove.Centro.X, circuloMove.Centro.Y), 0);
      pontoCentroCirculoMenor.ObjetoAtualizar();
    }

    private void VerificarColisao()
    {
      if (retanguloLimite == null) return;

      double x = circuloMove.Centro.X;
      double y = circuloMove.Centro.Y;
      double r = circuloMove.getRaio();

      double left = cx - semiLado;
      double right = cx + semiLado;
      double bottom = cy - semiLado;
      double top = cy + semiLado;

      bool encostouEsquerda = (x - r) <= left;
      bool encostouDireita = (x + r) >= right;
      bool encostouBaixo = (y - r) <= bottom;
      bool encostouCima = (y + r) >= top;

      if (encostouEsquerda || encostouDireita || encostouBaixo || encostouCima)
      {
        retanguloLimite.PrimitivaTipo = PrimitiveType.Points;
        retanguloLimite.PrimitivaTamanho = 0;
        retanguloLimite.ObjetoAtualizar();
      }
    }
